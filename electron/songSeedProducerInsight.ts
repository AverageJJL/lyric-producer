...
function clean(value: unknown): string | undefined {
  return typeof value === 'string' && value.trim().length > 6 ? value.trim() : undefined;
}

function role(name: string): 'intro' | 'verse' | 'pre' | 'chorus' | 'bridge' | 'outro' {
...
function contextPalette(context: string | undefined): string | null {
...
export function buildProducerInsight(input: ProducerInsightInput): ProducerInsight {
  const sectionRole = role(input.sectionName);
  const tilt = lyricTilt(input.lyrics);
  const palette = contextPalette(input.publicContext) ?? (input.hook
  ...
  const base: Record<typeof sectionRole, ProducerInsight> = {
    intro: {
  ...
  return base[sectionRole];
}

export function producerInsightFromUnknown(
  value: unknown,
  fallback: ProducerInsight,
): ProducerInsight {
  ...
  const next = {
    ...
    arrangementMove: clean(raw.arrangementMove),
    vocalTreatment: clean(raw.vocalTreatment),
    soundPalette: clean(raw.soundPalette),
    mixFocus: clean(raw.mixFocus),
    risk: clean(raw.risk),
  };
  return Object.values(next).every(Boolean) ? next as ProducerInsight : fallback;
}
