...
export function buildAudioArrangementShortcut(
  ...
  const maxSources = Math.max(1, Math.floor((AGENT_PATCH_MAX_CHANGES - 1) / (specs.length + 1)));
  ...
  sources.forEach(source => {
    ...
    specs.forEach(section => {
      const clip = clipForSection(source, section, usedIds, guideTrackId, dropout, muteDropout);
      if (clip) {
  ...
  const text = [
    ...
    'It adds muted guide slice blocks on new Build lanes while preserving the original audio for playback; no audio, MIDI, or new music is generated.',
  ].join(' ');
